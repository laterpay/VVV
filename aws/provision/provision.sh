...
if [[ $ping_result == *bytes?from* ]]; then
	...
	cp selenium-server-standalone-2.42.2.jar /usr/local/bin/selenium-server-standalone-2.42.2.jar

	#selenium start
	apt-get -y install firefox Xvfb Xorg
...
cp /srv/config/subversion-servers /home/ubuntu/.subversion/servers
if [[ ! -d /home/ubuntu/bin ]]; then
	mkdir /home/ubuntu/bin
fi
rsync -rvzh --delete /srv/config/homebin/ /home/ubuntu/bin/

echo " * /srv/config/bash_profile                      -> /home/ubuntu/.bash_profile"
echo " * /srv/config/bash_aliases                      -> /home/ubuntu/.bash_aliases"
echo " * /srv/config/vimrc                             -> /home/ubuntu/.vimrc"
echo " * /srv/config/subversion-servers                -> /home/ubuntu/.subversion/servers"
echo " * /srv/config/homebin                           -> /home/ubuntu/bin"

# If a bash_prompt file exists in the VVV config/ directory, copy to the VM.
if [[ -f /srv/config/bash_prompt ]]; then
	cp /srv/config/bash_prompt /home/ubuntu/.bash_prompt
	echo " * /srv/config/bash_prompt                       -> /home/ubuntu/.bash_prompt"
fi
...
if [[ "mysql: unrecognized service" != "${exists_mysql}" ]]; then
	...
	echo " * /srv/config/mysql-config/root-my.cnf          -> /home/ubuntu/.my.cnf"
...
if [[ $ping_result == *bytes?from* ]]; then
	...
	if [[ ! -d /srv/www/wp-cli ]]; then
		...
		chown -R ubuntu:ubuntu /srv/www/wp-cli/
		cd /srv/www/wp-cli
		composer install
	else
		echo -e "\nUpdating wp-cli..."
	...
	if [[ ! -d /srv/www/wordpress-default ]]; then
		...
		rm latest.tar.gz
		chown -R ubuntu:ubuntu /srv/www/wordpress-default/
		cd /srv/www/wordpress-default
		echo "Configuring WordPress Stable..."
		wp core config --dbname=wordpress_default --dbuser=wp --dbpass=wp --quiet --extra-php <<PHP
define( 'WP_DEBUG', true );
PHP
		#probably we should install wp manually
		wp core install --quiet --url=http://54.235.193.4/ --title="Local WordPress Dev" --admin_name=admin --admin_email="avahura@scnsoft.com" --admin_password="password"
